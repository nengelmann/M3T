...
int main(int argc, char *argv[]) {
  ...
  constexpr bool kVisualizeLinesCorrespondence = false;
  constexpr bool kVisualizePointsCorrespondence = false;
  constexpr bool kSaveImages = false;
  const std::filesystem::path save_directory{"./M3T/results/"};
  ...
  for (const auto body_name : body_names) {
    ...
    if(kVisualizeLinesCorrespondence) {
      region_modality_ptr->set_visualize_lines_correspondence(true);
    }
    if(kVisualizePointsCorrespondence) {
      region_modality_ptr->set_visualize_points_correspondence(true);
    }
    ...
    auto detector_ptr{std::make_shared<m3t::ManualDetector>(
      body_name + "manual_detector", detector_path, optimizer_ptr, color_camera_ptr)};
